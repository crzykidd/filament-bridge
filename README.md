...
- **Master defaults** — a variant family's parent (a real master or a colorless container) carries the shared fields its variants inherit in Filament DB: tare, nozzle/bed temperature, density, diameter, and material. Because a blank master leaves every un-overridden variant blank too — and makes the merge flow keep re-asking for values the family already knows — new masters are **seeded automatically** at creation with the family's most-common (mode) value, the **Master Defaults** page lets you **backfill existing** masters (review each proposed value per master; fills only what's currently null, never overwriting), and merging a new Spoolman spool into an existing family **pre-fills the tare from the incoming spool while showing the family's known value** so you can correct it in one click. See [master defaults](docs/master-defaults.md)
...
Latest tested upstreams: **Filament DB 1.67.0** and **Spoolman 0.24.0**.
