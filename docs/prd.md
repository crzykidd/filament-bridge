...
**Clarification — "conflicts are never auto-resolved":** this rule means the engine never silently picks a winning VALUE for a genuine data conflict (a field that changed on both sides). The three housekeeping paths above (`auto_stale_purge`, `auto_resolved_reappeared`, and `auto_resolved_converged`) are not value resolution — they clean up bridge-local conflict rows whose triggering condition no longer applies. `auto_resolved_converged` closes an open `cross_system` conflict once the sync pass has itself OBSERVED both sides' current values are equal (or both empty) — the divergence it was queued for no longer exists, so no value is chosen. It fires from a shared helper (`_auto_resolve_converged_conflicts`) wired into every `cross_system` producer (material-property scalars/temps, OpenTag extra fields, lifecycle, location, and the OpenPrintTag identity pass) — see `docs/decisions.md`, 2026-09-20 (#91). The user is not required to act on any of the three; all are logged in the sync audit trail.
