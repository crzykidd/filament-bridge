...
| **Location** | SM spool `location` (free-text name) ↔ FDB spool `locationId` (resolved to its **name**) | Compared **by name** for **mapped pairs only**. Spoolman stores the location as a string, does not trim it; Filament DB (≥1.76.0) trims `name` on its location model and migrates already-stored names on connect, so the bridge resolves `locationId` → name (one `GET /api/locations` per cycle) to diff and **finds-or-creates** the matching FDB location on a SM→FDB push (`core/locations.py:ensure_fdb_location`), trimming for the lookup key and the create so an untrimmed SM name still matches its (trimmed) FDB row. The FDB-side snapshot baseline records the trimmed value actually stored, never the raw SM string, so the whitespace difference isn't re-detected as a fresh change next cycle. The both-sides-converged check compares stripped names for the same reason (a whitespace-only difference must not queue a conflict). A one-sided change is a clean push; both sides changing to the *same* (stripped) name converges silently; both changing to genuinely *different* names queues a `cross_system` conflict with `field_name="location"` (using the raw values for display). Uses the `location_sync` category (`location_sync_direction` / `location_sync_conflict_policy`); `newest_wins` is not applicable (a name has no timestamp). Independent of weight — no ordering requirement. See GitHub #90. |
...
| **OpenTag identity** | SM extras `openprinttag_slug`/`uuid` ↔ FDB `settings{}` | Bidirectional, **baselined** (per-side `_opt_uuid`/`_opt_slug` merged into the same filament-level `Snapshot` row the multicolor/cost passes use). A side with no baseline that is currently empty gets **filled** from the other side (the original #81 behavior, direction-gated). A side whose baseline had a value that is now empty is a **deliberate unlink** (Filament DB ≥ 1.77.0's *Remove link*, or a direct Spoolman extra clear) — propagated to the other side rather than refilled, direction-gated, then both baselines refresh to the agreed value so the clear doesn't ping-pong back (#89). FDB writes stay scoped to `merge_filament_settings()` / `remove_filament_settings_keys()` (those two keys only). A genuine `uuid` divergence (both sides currently set, different) queues a `cross_system` conflict (`field_name="OpenPrintTag identity"`, deduped) — never auto-overwritten. Direction-gated on the `material_properties` category, same as the OPT material-setting extras. |
